...
class Controller extends BaseController
{
    ...
    /**
     * Record prepaid campaign to failed_prepaid_campaignid when payment fails
     * 
     * @param User $updateUser
     * @param string $leadspeek_api_id
     * @param string $paymentterm
     * @return void
     */
    public function recordPrepaidCampaignsStopContinualTopup($updateUser, $leadspeek_api_id, $paymentterm = 'Prepaid')
    {
        Log::info("recordPrepaidCampaignsStopContinualTopup", ['get_defined_vars' => get_defined_vars()]);
        try{
            /* VALIDATION */
            if(empty($updateUser) || empty($leadspeek_api_id) || trim($paymentterm) != 'Prepaid'){
                return;
            }
            /* VALIDATION */
    
            /* VARIABLE */
            $leadspeekApiId = trim($leadspeek_api_id);
            $existingFailedPrepaid = trim($updateUser->failed_prepaid_campaignid ?? '');
            $existingIds = !empty($existingFailedPrepaid) ? explode('|', $existingFailedPrepaid) : []; // Explode to array
            /* VARIABLE */
            
            /* ADD NEW ID IF NOT EXISTS */
            if(!in_array($leadspeekApiId, $existingIds)){
                $existingIds[] = $leadspeekApiId;
            }
            /* ADD NEW ID IF NOT EXISTS */
            
            /* FILTER EMPTY VALUES AND IMPLODE */
            $existingIds = array_filter(array_map('trim', $existingIds));
            $updateUser->failed_prepaid_campaignid = !empty($existingIds) ? implode('|', $existingIds) : '';
            $updateUser->save();
            /* FILTER EMPTY VALUES AND IMPLODE */
        }catch(\Throwable $th){
            Log::error('Error recordPrepaidCampaignsStopContinualTopup: ' . $th->getMessage());
        }
    }

    /**
     * Resume prepaid campaigns stopcontinual from T to F and clear failed_prepaid_campaignid
     * 
     * @param User $updateUser
     * @param int|null $updated_by_user_id For logging
     * @param string $source Source of the action (e.g., 'campaign cost month', 'auto topup campaign')
     * @return void
     */
    public function resumePrepaidCampaignsStopContinualTopup($updateUser, $updated_by_user_id = null, $source = '')
    {
        Log::info("resumePrepaidCampaignsStopContinualTopup", ['get_defined_vars' => get_defined_vars()]);
        try{
            /* VALIDATION */
            if(empty($updateUser) || empty($updateUser->failed_prepaid_campaignid)){
                return;
            }
            
            $failedPrepaidCampaignIds = trim($updateUser->failed_prepaid_campaignid ?? '');
            if(empty($failedPrepaidCampaignIds)){
                return;
            }
            
            $campaignIds = array_filter(array_map('trim', explode('|', $failedPrepaidCampaignIds))); // Explode to array
            if(empty($campaignIds)) {
                return;
            }
            /* VALIDATION */
            
            /* RESUME PREPAID CAMPAIGNS */
            // Update stopcontinual from T to F for prepaid continual campaigns
            LeadspeekUser::whereIn('leadspeek_api_id', $campaignIds)
                ->update(['stopcontinual' => 'F']);
            
            // Log the action for each campaign (from API system)
            if(!empty($updated_by_user_id) && !empty($campaignIds)){
                $title = 'Resume Continual Top-up';
                foreach($campaignIds as $campaignId){
                    $description = 'Resume continual top-up for prepaid campaign : ' . $campaignId . ' | userID : ' . $updateUser->id . ' | from API system';
                    if(!empty($source)){
                        $description .= ' | source : ' . $source;
                    }
                    $this->logUserAction($updated_by_user_id, $title, $description, '', $updateUser->id);
                }
            }
            
            // Clear failed_prepaid_campaignid after reset
            $updateUser->failed_prepaid_campaignid = '';
            /* RESUME PREPAID CAMPAIGNS */
            
            $updateUser->save();
        }catch(\Throwable $th){
            Log::error('Error resumePrepaidCampaignsStopContinualTopup: ' . $th->getMessage());
        }
    }
    ...
    public function UpsertFailedLeadRecord(array $data, $id = '') 
    {
        ...
        {
            Log::error('Error UpsertFailedLeadRecord: ' . $e->getMessage(), [
    ...
    public function transfer_commission_sales($companyParentID,$platformfee,$_leadspeek_api_id = "",$startdate = "0000-00-00 00:00:00",$enddate = "0000-00-00 00:00:00",$stripeseckey = "",$ongoingleads = "",$cleanProfit = "",$dataCustomCommissionSales = [],$transfer_group = "",$source_transaction = "") 
    {
        ...
        if (count($saleslist) > 0 && $platformfee > 0 && $salesfee > 0) 
        {
            ...
            {
                $_leadspeek_type = 'wallet';
            }
            // info(['_leadspeek_type' => $_leadspeek_type]);
            /** GET OTHER DETAILS */
            ...
            foreach($saleslist as $sale) 
            {
                ...
                if ($currbalance >= $salesfee) 
                {
                    ...
                    $description = ($chargeType == 'wallet') ?
                                   ("Commision from {$sale['company_name']} for data wallet {$chargeFrom}") : 
                                   ("Commision from {$sale['company_name']} for campaign #{$_leadspeek_api_id}") ;
        ...
        if(count($chkUser) > 0) 
        {
                ...
                if(count($leadsuser) > 0)
                {
                    foreach($leadsuser as $lds) 
                    {
                        ...
                        if(!($lds['active'] == "F" && $lds['disabled'] == "T" && $lds['active_user'] == "F")) 
                        {
                            ...
                            $this->recordPrepaidCampaignsStopContinualTopup($updateUser, $_leadspeek_api_id, $_campaign_paymentterm);
                            /** UPDATE USER CARD STATUS */
                        }
                    }
                }
            ...
            else 
            {
                ...
                if($updateUser && empty($updateUser->failed_campaignid) && empty($updateUser->failed_total_amount))
                {
                    ...
                    $this->resumePrepaidCampaignsStopContinualTopup($updateUser, $chkUser[0]['id'] ?? null, 'auto topup campaign');
                }
                // clear payment status client
            }
            /* CHECK IF AGENCY PAYMENT FAILED  OR NOT FAILED */
    ...
    public function process_charge_agency_wallet($companyParentID, $platformfee, $_campaign_paymentterm, &$topupAgencyExists, &$remainingPlatformfee, &$statusPayment, &$statusPaymentClient, &$errorstripeClient, &$topup_agencies_id, &$leadspeek_invoices_id, &$chargeStatusWithPrepaid, &$applicationFeeAmount, $_agency_stop_continual, $_agency_amount, $_agency_custom_amount, $_agency_payment_type, $_agency_ip_user, $_agency_timezone, $_agency_balance_threshold, $stripeseckey, $chkUser, $defaultInvoice, $transferGroup, $_leadspeek_api_id, $_agency_name, $_client_name, $_campaign_name, $dataChargeClient)
    {
        ...
        DB::transaction(function () use ($companyParentID, $platformfee, $_campaign_paymentterm, &$topupAgencyExists, &$remainingPlatformfee, &$statusPayment, &$statusPaymentClient, &$errorstripeClient, &$topup_agencies_id, &$leadspeek_invoices_id, &$chargeStatusWithPrepaid, &$applicationFeeAmount, $_agency_stop_continual, $_agency_amount, $_agency_custom_amount, $_agency_payment_type, $_agency_ip_user, $_agency_timezone, $_agency_balance_threshold, $stripeseckey, $chkUser, $defaultInvoice, $transferGroup, $_leadspeek_api_id, $_agency_name, $_client_name, $_campaign_name, $dataChargeClient) {
            ...
            if($balanceAmount > 0 && $agencyAgreeDataWallet == 'T') // balance harus di atas 0 dan harus agree data wallet
            {
                ...
                if($agencyManualBill == 'F' && ($_agency_stop_continual == 'T' || !$isEnableTopupPrepaidDirectPayment || $_agency_amount <= 0) && $balanceAmount < $platformfee && $amountClient >= 0.5)
                {
                ...
                if($agencyManualBill == 'F' && $amountClient >= 0.5 && $chargeStatusWithPrepaid == 'paid')
                {
    ...
    public function agencyzoom_sendrecord($webhook = "",$FirstName = "",$LastName = "",$BusinessName = "",$Email = "",$Email2 = "",$Phone = "",$Phone2 = "",$Address = "",$Address2 = "",$City = "",$State = "",$Zipcode = "",$Keyword = "",$campaignID = "", $leadspeek_type = "", $md5param = "", $customParams = "")
    {
        ...
        // Hanya tambahkan customParams untuk campaign local
        if ($leadspeek_type === 'local') {
            $contact_data["customParams"] = $customParams;
        }
        ...
        catch (\GuzzleHttp\Exception\BadResponseException $e) 
        {
            /* WRITE UPSER FAILED LEAD RECORD */
            $message = $e->getMessage();
            Log::error("agencyzoom_sendrecord error", [
                'message' => $message
            ]);
            $this->UpsertFailedLeadRecord([
