  ...
  <div
    ...
    <sidebar-toggle-button />
      <template  slot="links">
        ...
        <sidebar-item
          class="--sidebar-custom-item"
          :showBeta="this.$global.betaFeature.predict_module.is_beta && (this.$global.betaFeature.predict_module.apply_to_all_agency || this.$global.isBeta)"
          :link="{
            name: this.$global.globalModulNameLink.predict.name,
            icon: 'fa-regular fa-crystal-ball',
            path: '/' + encodeURIComponent(this.$global.globalModulNameLink.predict.url) + '/campaign-management'
          }"
          v-if="((this.$global.menuLeadsPeek && this.$global.menuUserType != 'client' && (!this.$global.agencyPaymentFailed || this.$global.globalviewmode) && !this.$global.systemUser && this.$global.creditcardsetup && this.$global.stripeaccountconnected && this.activesidebar['predict']) || (this.$global.menuLeadsPeek && this.$global.menuLeadsPeek_typeLocator == 'locator' && this.$global.menuUserType == 'client' && this.$global.stripeaccountconnected && !this.$global.systemUser && this.activesidebar['predict'])) && (this.$global.globalModulNameLink.predict.name != null) && (this.$global.globalModulNameLink.predict.url != null) && (this.$store.getters.userData.company_root_id == this.$global.masteridsys) && (this.validateBetaFeature('predict_module'))">
        </sidebar-item>
        <!-- Predict ID -->
        ...
        <sidebar-item
          ...
          v-if="(this.$global.menuLeadsPeek && this.$global.menuUserType != 'client' && this.$global.creditcardsetup && this.$global.stripeaccountconnected && this.$global.idsys == this.$global.masteridsys && this.$global.apiMode && this.$global.is_marketing_services_agreement_developer && this.validateBetaFeature('clean_module') && (!this.$global.agencyPaymentFailed || this.$global.globalviewmode))">
...
export default {
  ...
  methods: {
    validateBetaFeature(type) {
      ...
      } else if (type == 'predict_module') {
        if (!this.$global.betaFeature.predict_module.is_beta || this.$global.betaFeature.predict_module.apply_to_all_agency || this.$global.isBeta) {
          return true;
        } else {
          return false;
        }
